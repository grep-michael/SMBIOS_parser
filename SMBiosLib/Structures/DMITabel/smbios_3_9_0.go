...
func (s *Type0PlatformFirmwareInformation) GetFirmwareCharacteristicsFlags() []string {
	...
	if v&(1<<3) != 0 {
		flags = append(flags, "Firmware Characteristics are not supported")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "ISA is supported")
	}
	if v&(1<<5) != 0 {
		flags = append(flags, "MCA is supported")
	}
	if v&(1<<6) != 0 {
		flags = append(flags, "EISA is supported")
	}
	if v&(1<<7) != 0 {
		flags = append(flags, "PCI is supported")
	}
	if v&(1<<8) != 0 {
		flags = append(flags, "PC card (PCMCIA) is supported")
	}
	if v&(1<<9) != 0 {
		flags = append(flags, "Plug and Play is supported")
	}
	if v&(1<<10) != 0 {
		flags = append(flags, "APM is supported")
	}
	if v&(1<<11) != 0 {
		flags = append(flags, "Firmware is upgradeable (Flash)")
	}
	if v&(1<<12) != 0 {
		flags = append(flags, "Firmware shadowing is allowed")
	}
	if v&(1<<13) != 0 {
		flags = append(flags, "VL-VESA is supported")
	}
	if v&(1<<14) != 0 {
		flags = append(flags, "ESCD support is available")
	}
	if v&(1<<15) != 0 {
		flags = append(flags, "Boot from CD is supported")
	}
	if v&(1<<16) != 0 {
		flags = append(flags, "Selectable boot is supported")
	}
	if v&(1<<17) != 0 {
		flags = append(flags, "Firmware ROM is socketed (e")
	}
	if v&(1<<18) != 0 {
		flags = append(flags, "Boot from PC card (PCMCIA) is supported")
	}
	if v&(1<<19) != 0 {
		flags = append(flags, "EDD specification is supported")
	}
	if v&(1<<20) != 0 {
		flags = append(flags, "Int 13h — Japanese floppy for NEC 9800 1")
	}
	if v&(1<<21) != 0 {
		flags = append(flags, "Int 13h — Japanese floppy for Toshiba 1")
	}
	if v&(1<<22) != 0 {
		flags = append(flags, "Int 13h — 5")
	}
	if v&(1<<23) != 0 {
		flags = append(flags, "Int 13h — 5")
	}
	if v&(1<<24) != 0 {
		flags = append(flags, "Int 13h — 3")
	}
	if v&(1<<25) != 0 {
		flags = append(flags, "Int 13h — 3")
	}
	if v&(1<<26) != 0 {
		flags = append(flags, "Int 5h, print screen Service is supported")
	}
	if v&(1<<27) != 0 {
		flags = append(flags, "Int 9h, 8042 keyboard services are supported")
	}
	if v&(1<<28) != 0 {
		flags = append(flags, "Int 14h, serial services are supported")
	}
	if v&(1<<29) != 0 {
		flags = append(flags, "Int 17h, printer services are supported")
	}
	if v&(1<<30) != 0 {
		flags = append(flags, "Int 10h, CGA/Mono Video Services are supported")
	}
	if v&(1<<31) != 0 {
		flags = append(flags, "NEC PC-98")
	}
	return flags
}
...
func (s *Type0PlatformFirmwareInformation) GetFirmwareCharacteristicsExtensionBytesFlags() []string {
	var flags []string
	// Byte 1 (offset 12h)
	b0 := s.FirmwareCharacteristicsExtensionBytes[0]
	if b0&(1<<0) != 0 {
		flags = append(flags, "ACPI is supported")
	}
	if b0&(1<<1) != 0 {
		flags = append(flags, "USB Legacy is supported")
	}
	if b0&(1<<2) != 0 {
		flags = append(flags, "AGP is supported")
	}
	if b0&(1<<3) != 0 {
		flags = append(flags, "I2O boot is supported")
	}
	if b0&(1<<4) != 0 {
		flags = append(flags, "LS-120 SuperDisk boot is supported")
	}
	if b0&(1<<5) != 0 {
		flags = append(flags, "ATAPI ZIP drive boot is supported")
	}
	if b0&(1<<6) != 0 {
		flags = append(flags, "1394 boot is supported")
	}
	if b0&(1<<7) != 0 {
		flags = append(flags, "Smart battery is supported")
	}
	// Byte 2 (offset 13h)
	b1 := s.FirmwareCharacteristicsExtensionBytes[1]
	if b1&(1<<0) != 0 {
		flags = append(flags, "BIOS Boot Specification is supported")
	}
	if b1&(1<<1) != 0 {
		flags = append(flags, "Function key-initiated network service boot is supported")
	}
	if b1&(1<<2) != 0 {
		flags = append(flags, "Enable targeted content distribution")
	}
	if b1&(1<<3) != 0 {
		flags = append(flags, "UEFI Specification is supported")
	}
	if b1&(1<<4) != 0 {
		flags = append(flags, "SMBIOS table describes a virtual machine")
	}
	if b1&(1<<5) != 0 {
		flags = append(flags, "Manufacturing mode is supported")
	}
	if b1&(1<<6) != 0 {
		flags = append(flags, "Manufacturing mode is enabled")
	}
	if b1&(1<<7) != 0 {
		flags = append(flags, "Reserved for future assignment by this specification")
	}
	return flags
}
...
var Type4_ProcessorFamilyMap = map[uint16]string{
	0x01:  "Other",
	0x02:  "Unknown",
	0x03:  "8086",
	0x04:  "80286",
...
func (s *Type4ProcessorInformation) GetProcessorFamilyString() string {
	fam := uint16(s.ProcessorFamily)
	if fam > 0 {
		if v, ok := Type4_ProcessorFamilyMap[fam]; ok {
			return v
		}
	} else {
		if v, ok := Type4_ProcessorFamilyMap[s.ProcessorFamily2]; ok {
			return v
		}
	}
	return "Unknown"
...
func (s *Type5MemoryController) GetErrorCorrectingCapabilityFlags() []string {
	...
	if v&(1<<3) != 0 {
		flags = append(flags, "Single-Bit Error Correcting")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "Double-Bit Error Correcting")
	}
	if v&(1<<5) != 0 {
		flags = append(flags, "Error Scrubbing")
	}
	return flags
}
...
func (s *Type5MemoryController) GetSupportedSpeedsFlags() []string {
	...
	if v&(1<<2) != 0 {
		flags = append(flags, "70ns")
	}
	if v&(1<<3) != 0 {
		flags = append(flags, "60ns")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "50ns")
	}
	return flags
}
...
func (s *Type6MemoryModuleConfiguration) GetCurrentMemoryTypeFlags() []string {
	...
	if v&(1<<3) != 0 {
		flags = append(flags, "Fast Page Mode")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "EDO")
	}
	if v&(1<<5) != 0 {
		flags = append(flags, "Parity")
	}
	if v&(1<<6) != 0 {
		flags = append(flags, "ECC")
	}
	if v&(1<<7) != 0 {
		flags = append(flags, "SIMM")
	}
	if v&(1<<8) != 0 {
		flags = append(flags, "DIMM")
	}
	if v&(1<<9) != 0 {
		flags = append(flags, "Burst EDO")
	}
	if v&(1<<10) != 0 {
		flags = append(flags, "SDRAM")
	}
	return flags
}
...
func (s *Type17MemoryDevice) GetTypeDetailFlags() []string {
	...
	var flags []string
	if v&(1<<0) != 0 {
		flags = append(flags, "Reserved, set to 0")
	}
	if v&(1<<1) != 0 {
		flags = append(flags, "Other")
	}
	...
	if v&(1<<3) != 0 {
		flags = append(flags, "Fast-paged")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "Static column")
	}
	if v&(1<<5) != 0 {
		flags = append(flags, "Pseudo-static")
	}
	if v&(1<<6) != 0 {
		flags = append(flags, "RAMBUS")
	}
	if v&(1<<7) != 0 {
		flags = append(flags, "Synchronous")
	}
	if v&(1<<8) != 0 {
		flags = append(flags, "CMOS")
	}
	if v&(1<<9) != 0 {
		flags = append(flags, "EDO")
	}
	if v&(1<<10) != 0 {
		flags = append(flags, "Window DRAM")
	}
	if v&(1<<11) != 0 {
		flags = append(flags, "Cache DRAM")
	}
	if v&(1<<12) != 0 {
		flags = append(flags, "Non-volatile")
	}
	if v&(1<<13) != 0 {
		flags = append(flags, "Registered (Buffered)")
	}
	if v&(1<<14) != 0 {
		flags = append(flags, "Unbuffered (Unregistered)")
	}
	if v&(1<<15) != 0 {
		flags = append(flags, "LRDIMM")
	}
	return flags
}
...
func (s *Type17MemoryDevice) GetMemoryOperatingModeCapabilityFlags() []string {
	...
	if v&(1<<3) != 0 {
		flags = append(flags, "Volatile memory")
	}
	if v&(1<<4) != 0 {
		flags = append(flags, "Byte-accessible persistent memory")
	}
	if v&(1<<5) != 0 {
		flags = append(flags, "Block-accessible persistent memory")
	}
	if v&(1<<6) != 0 {
		flags = append(flags, "Reserved, set to 0")
	}
